...
func (r *ClusterScan) validateClusterScan(ctx context.Context, c client.Client) field.ErrorList {
	...
	if r.Spec.NodeScanTemplate != nil {
		...
		// Validate excludePatterns in template — these are propagated to every
		// NodeScan generated by this ClusterScan, so invalid patterns must be
		// caught here rather than failing one NodeScan at a time.
		if len(r.Spec.NodeScanTemplate.ExcludePatterns) > 0 {
			allErrs = append(allErrs, ValidateExcludePatterns(
				r.Spec.NodeScanTemplate.ExcludePatterns,
