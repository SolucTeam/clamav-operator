...
HELM ?= helm
HELM_CHART_DIR ?= helm/clamav-operator
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f build/Dockerfile .
...
helm-template: ## Template Helm chart
	$(HELM) template clamav-operator $(HELM_CHART_DIR) \
...
helm-install: ## Install Helm chart
	$(HELM) install clamav-operator $(HELM_CHART_DIR) \
		--namespace clamav-system \
		--create-namespace
...
helm-upgrade: ## Upgrade Helm chart
	$(HELM) upgrade clamav-operator $(HELM_CHART_DIR) \
		--namespace clamav-system
...
helm-docs: ## Generate Helm chart documentation
	...
	@echo "See $(HELM_CHART_DIR)/README.md"
